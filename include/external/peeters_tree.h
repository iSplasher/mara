...
tree<T, tree_node_allocator>::begin_fixed( const iterator_base& pos, unsigned int dp, bool walk_back ) const {
...
tree<T, tree_node_allocator>::path_from_iterator( const iterator_base& iter, const iterator_base& top ) const {
  path_t     path;
...
tree<T, tree_node_allocator>::iterator_from_path( const path_t& path, const iterator_base& top ) const {
  iterator   it   = top;
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<T, tree_node_allocator>::begin_leaf( const iterator_base& top
) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<T, tree_node_allocator>::end_leaf( const iterator_base& top
) const {
...
template<class T, class tree_node_allocator>
template<typename iter, class BinaryPredicate>
bool tree<T, tree_node_allocator>::equal( const iter& one_, const iter& two, const iter& three_, BinaryPredicate fun )
  const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::iterator
tree<T, tree_node_allocator>::lowest_common_ancestor( const iterator_base& one, const iterator_base& two ) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::sibling( const iterator_base& it, unsigned int num ) const {
  tree_node* tmp;
  if ( it.node->parent == 0 ) {
